...
public class ScanTask {
    ...
    private static class LazyHolder {
        private static final ScanTask INSTANCE = new ScanTask();
    }

    private ScanTask() {
    }

    public static final ScanTask getInstance() {
        return LazyHolder.INSTANCE;
    }

    public static Set<Class<?>> packageList = new HashSet<Class<?>>();
    ...
    private static String scanPath = ScanConfig.getScanRoot();

    /**
     * 快速扫描指定路径下的class文件 查找注解
     */
    public synchronized static void scanInit() {
        logger.debug("注解扫描启动，扫描列表：" + scanPath);
        String[] arr = null;
        try {
            arr = scanPath.split(",");
        } catch (Exception e) {
            CustomException.throwOut("scanInit path Unable to resolve", e);
        } finally {
            scanPath = null;
        }
        classLoader = BeanFactory.getClassLoader();
        for (String s : arr) {
